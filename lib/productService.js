...
const CLOUD_NAME = 'diamzzlzs';
const cloudinaryPrefix = '/https://res.cloudinary.com/${CLOUD_NAME}/image/fetch/f_auto,q_auto/';
/**
...
function normalizeProduct(item) {
  ...
  if (Array.isArray(item.images)) {
    normalizedImages = item.images.filter(Boolean);
  } else if (typeof item.images === 'string' && item.images.trim()) {
    try {
      const parsedImages = JSON.parse(item.images);
      normalizedImages = Array.isArray(parsedImages) ? parsedImages.filter(Boolean) : [item.images];
    } catch {
      normalizedImages = [item.images];
    }
  }

  // Đảm bảo luôn có image - ưu tiên image riêng, rồi đến phần tử đầu của images
  const imageUrl = item.image || item.imageUrl || item.thumbnail || normalizedImages[0] || '';
 ...
 * @param {File} file 
 * @returns {Promise<string|null>} URL của ảnh đã upload
 */
export async function uploadProductImage(file) {
  try {
    ...
    if (uploadError) {
      console.error('[ProductService] Lỗi upload ảnh:', uploadError);
    ...
    console.error('[ProductService] Exception khi upload ảnh:', err);
    throw err;
  }
}
...
export async function createProduct(productData) {
  ...
  if (error) {
    console.error('[ProductService] Lỗi tạo sản phẩm:', error);
    throw error;
  }
  return normalizeProduct(data);
}
...
export async function updateProduct(id, productData) {
  ...
  if (error) {
    console.error(`[ProductService] Lỗi cập nhật sản phẩm ${id}:`, error);
    throw error;
  }
  return normalizeProduct(data);
}
...
export async function deleteProduct(id) {
  ...
  if (error) {
    console.error(`[ProductService] Lỗi xoá sản phẩm ${id}:`, error);
    throw error;
  }
}
