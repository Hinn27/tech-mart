...
import { tabletProducts, formatPrice } from '@/lib/mock-data';

const ACCENT = '#7C3AED';

export default function MayTinhBangPage() {
  ...
  const filteredProducts = tabletProducts.filter((product) => {
    if (selectedBrand !== 'all') {
      const matchesBrand =
        (selectedBrand === 'Apple' && product.name.includes('iPad')) ||
        (selectedBrand === 'Samsung' && product.name.includes('Samsung')) ||
        (selectedBrand === 'Xiaomi' && product.name.includes('Xiaomi')) ||
        (selectedBrand === 'Huawei' && product.name.includes('Huawei'));
      if (!matchesBrand) return false;
    }

    if (priceRange === 'under15') return product.price < 15000000;
    if (priceRange === '15to25') return product.price >= 15000000 && product.price <= 25000000;
    if (priceRange === '25to35') return product.price > 25000000 && product.price <= 35000000;
    if (priceRange === 'over35') return product.price > 35000000;
  ...
  const breadcrumbItems = [{ label: 'Máy tính bảng', href: '/may-tinh-bang' }];
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1400px] px-4 py-6">
        ...
        <div
          ...
          style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, #6D28D9 50%, #5B21B6 100%)` }}
        >
          ...
          <div className="relative z-10 flex flex-col justify-center h-full px-8 md:px-12 text-white">
            <p className="text-sm md:text-base font-medium opacity-90 mb-1">
              Máy tính bảng cao cấp — Làm việc & Giải trí
            </p>
            ...
            <p className="text-sm md:text-base opacity-80 mt-2 max-w-lg hidden sm:block">
              iPad Pro M4, Galaxy Tab S9 Ultra, Xiaomi Pad 6S Pro — Giá tốt nhất, bảo hành chính hãng, giao hàng nhanh.
            </p>
          </div>
        </div>
            ...
            <div className="sticky top-20 space-y-5">
              ...
              <div className="rounded-xl bg-card border border-border p-4">
                ...
                <div className="space-y-2">
                  {[
                    { id: 'all', name: 'Tất cả' },
                    { id: 'under15', name: `Dưới ${formatPrice(15000000)}` },
                    { id: '15to25', name: `${formatPrice(15000000)} - ${formatPrice(25000000)}` },
                    { id: '25to35', name: `${formatPrice(25000000)} - ${formatPrice(35000000)}` },
                    { id: 'over35', name: `Trên ${formatPrice(35000000)}` },
                  ].map((range) => (
              ...
              <div className="lg:hidden mb-4 rounded-xl bg-card border border-border p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      { id: 'all', name: 'Tất cả' },
                      { id: 'under15', name: 'Dưới 15Tr' },
                      { id: '15to25', name: '15Tr - 25Tr' },
                      { id: '25to35', name: '25Tr - 35Tr' },
                      { id: 'over35', name: 'Trên 35Tr' },
                    ].map((range) => (
                      <button
                        key={range.id}
